...
#   CLASSIFIER ON REAL VALUES
    ...
    curve_0_y_ = np.asarray([np.arange(values)]*40)

    test_data_classifier_curve_0 = np.concatenate((curve_0_x[sample_0:sample_0+40], curve_0_y_), axis=1)
    test_data_classifier_curve_1 = np.concatenate((curve_1_x[sample_1:sample_1+40], curve_0_y_), axis=1)
    test_data = np.concatenate((test_data_classifier_curve_0, test_data_classifier_curve_1), axis=0)
    label_x_test = [[int(i)] for i in np.zeros(40)]
    label_y_test = [[int(i)] for i in np.ones(40)]
