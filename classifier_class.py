...
class classifier():
    ...
    def cnn_model_fn(self, features, labels, mode):
        ...
        dense = tf.layers.dense(inputs=pool2_flat, units=10 * 25 * self.num_values, activation=tf.nn.relu)
    ...
    def train(self, train_data, labels, cl, epochs):
        ...
        train_input_fn = tf.estimator.inputs.numpy_input_fn(x={'x': train_data}, y=labels, batch_size=10, num_epochs=None, shuffle=True)
        cl.train(input_fn=train_input_fn, steps=epochs, hooks=[logging_hook])
                                                  
    def valuate(self, eval_data, labels, cl):
        eval_input_fn = tf.estimator.inputs.numpy_input_fn(x={"x": eval_data}, y=labels, num_epochs=1, shuffle=False)
